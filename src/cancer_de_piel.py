...
## Cargar dataset
## =========================
df = pd.read_csv(r"C:\Users\ejoji\Downloads\datasets2\Cancer de piel\HAM10000_metadata.csv")
...
df = df.drop(columns=["lesion_id", "image_id", "dx_type"])
...
# Separar X y y
X = df.drop(columns=["dx"]).values.astype(np.float32)
y = df["dx"].values.astype(np.int32)
